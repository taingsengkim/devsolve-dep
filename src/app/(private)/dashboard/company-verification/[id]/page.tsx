  ...
  const handleConfirmApprove = async () => {
    ...
    try {
      ...
      toast.success("Application Approved!", {
        description: `${company.companyName} is now verified and active.`,
      });
      refetchRealOrg();
    } catch (err: unknown) {
  ...
  const handleConfirmReject = async () => {
    ...
    try {
      ...
      toast.success("Application Rejected", {
        description: `${company.companyName} verification application has been rejected.`,
      });
      refetchRealOrg();
    } catch (err: unknown) {
      const errorData = err as { data?: { message?: string } };
    ...
    >
      ...
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-slate-200/80 dark:border-slate-800">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-3">
              ...
              {company.orgCode && (
                <button
                  ...
                  className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-slate-100 dark:bg-slate-800 text-xs font-mono text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 transition-colors cursor-pointer"
                  title="Copy slug"
                >
                  {copiedField === "Slug" ? (
            ...
            <div className="flex flex-wrap items-center gap-2 pt-0.5">
              ...
              {company.emailVerified && (
                <Badge className="bg-emerald-50 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800/60 gap-1 text-xs px-2.5 py-0.5 rounded-full font-medium">
        ...
        <div className="text-left sm:text-right shrink-0">
          <span className="text-xs text-slate-400 font-medium block">Submitted Date</span>
      ...
      {company.status === "APPROVED" && (
        <motion.div
          ...
          className="p-4 rounded-2xl bg-emerald-50/80 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800/60 flex items-start gap-3 text-emerald-900 dark:text-emerald-200 shadow-2xs"
        >
      ...
      {company.status === "REJECTED" && (
        <motion.div
          initial={{ opacity: 0, scale: 0.98 }}
          animate={{ opacity: 1, scale: 1 }}
          className="p-4 rounded-2xl bg-rose-50/80 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-800/60 flex items-start gap-3 text-rose-900 dark:text-rose-200 shadow-2xs"
        >
          <XCircle className="w-5 h-5 text-rose-600 dark:text-rose-400 shrink-0 mt-0.5" />
          <div className="space-y-1 flex-1">
            <h4 className="text-sm font-bold">Verification Application Rejected</h4>
            {company.rejectionReason && (
              <div className="p-3 bg-white/70 dark:bg-slate-900/60 rounded-xl text-xs font-medium text-rose-800 dark:text-rose-300 border border-rose-200/80 dark:border-rose-900/50">
                <span className="font-bold block text-[11px] uppercase tracking-wider text-rose-500 mb-0.5">
                  Rejection Reason:
                </span>
          ...
          <Card className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-2xs">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-3">
                {isFinalized ? (
                  <Badge variant="outline" className="px-3 py-1 text-xs font-semibold text-slate-500">
                    Decision Finalized
                  </Badge>
                ) : (
                  <>
