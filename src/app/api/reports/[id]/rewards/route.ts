...
async function proxyRewards(
  ...
  const targetUrl = `${baseUrl}/reports/${id}/rewards`;
  ...
  try {
    ...
    if (!upstream.ok) {
      const message =
        ...
        "Failed to update report reward information.";
    ...
    return NextResponse.json(
      { message: "Unable to reach the report rewards service. Please try again." },
      { status: 502 }
    );
  }
}
...
export async function POST(
  ...
  return proxyRewards(request, context.params);
}
