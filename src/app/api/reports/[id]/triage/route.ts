...
async function proxyTriage(
  ...
  const targetUrl = `${baseUrl}/reports/${id}/triage`;
  ...
  try {
    ...
    if (!upstream.ok) {
      const message =
        ...
        "Failed to update report triage status.";
    ...
    return NextResponse.json(
      { message: "Unable to reach the report triage service. Please try again." },
      { status: 502 }
    );
  }
}
...
export async function POST(
  ...
  return proxyTriage(request, context.params);
}
