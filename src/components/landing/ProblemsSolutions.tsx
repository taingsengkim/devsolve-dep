...
function StatePill({ solved }: { solved: boolean }) {
  return solved ? (
    ...
    <span className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 px-2.5 py-1 text-xs font-bold text-slate-500 dark:border-slate-700 dark:text-slate-400">
...
export function ProblemsSolutions() {
  ...
  return (
    <section
      ref={ref}
      className="relative overflow-hidden bg-white py-20 sm:py-24 dark:bg-slate-950"
    >
      ...
      <div className="relative mx-auto w-full max-w-7xl px-6 sm:px-12">
        ...
        <div className="mt-10 grid grid-cols-1 gap-10 lg:grid-cols-12 lg:gap-12">
          ...
          >
            ...
            <dl className="mt-8 grid grid-cols-2 gap-x-8 gap-y-6 border-t border-slate-200 pt-8 dark:border-slate-800">
              {MINI_STATS.map((s) => (
                <div key={s.label}>
            ...
            <Link
              href="/problems"
              className="group mt-8 inline-flex items-center gap-2 rounded-full bg-[#1E293B] px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:brightness-110 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-white"
            >
          ...
          <div className="lg:col-span-8">
            <div className="relative">
              ...
              <motion.article
                ...
                className="rounded-2xl bg-white p-6 shadow-[0_0_0_1px_rgba(30,41,59,0.08),0_2px_10px_rgba(30,41,59,0.05)] dark:bg-slate-900 dark:shadow-[0_0_0_1px_rgba(148,163,184,0.14),0_2px_10px_rgba(2,6,23,0.5)]"
              >
                <div className="flex flex-wrap items-center gap-2">
                  <span className="rounded-lg bg-slate-100 px-2.5 py-1 text-xs font-bold uppercase tracking-[0.14em] text-slate-500 dark:bg-slate-800 dark:text-slate-300">
                    Problem
                  </span>
                  {PROBLEM.tags.map((t) => (
                    <span
                      key={t}
                      className="rounded-lg border border-slate-200 px-2 py-0.5 text-xs font-medium text-slate-500 dark:border-slate-700 dark:text-slate-400"
                    >
                      {t}
                    </span>
                  ))}
                ...
                <div className="mt-5 flex items-center gap-4 text-xs text-slate-400 dark:text-slate-500">
                  <span className="flex items-center gap-1.5">
                    ...
                    <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[#1E293B] text-[10px] font-bold text-white dark:bg-slate-700">
              ...
              <motion.article
                ...
                className="ml-0 mt-6 rounded-2xl bg-white p-6 shadow-[0_0_0_1px_rgba(16,185,129,0.3),0_2px_10px_rgba(30,41,59,0.05)] sm:ml-12 dark:bg-slate-900 dark:shadow-[0_0_0_1px_rgba(16,185,129,0.4),0_2px_10px_rgba(2,6,23,0.5)]"
              >
                <div className="flex flex-wrap items-center gap-2">
                  <StatePill solved />
                  <span className="text-xs font-medium text-slate-400 dark:text-slate-500">
                ...
                <pre className="mt-4 overflow-x-auto rounded-xl bg-slate-50 px-4 py-3 text-xs leading-relaxed text-slate-600 dark:bg-slate-950 dark:text-slate-300">
            ...
            <div className="mt-10">
              <div className="flex items-baseline justify-between border-b border-slate-200 pb-3 dark:border-slate-800">
                ...
                <Link
                  href="/problems"
                  className="text-xs font-semibold text-slate-400 transition-colors hover:text-slate-700 dark:text-slate-500 dark:hover:text-slate-200"
                >
                  See all
                </Link>
              </div>
              ...
              {THREADS.map((t, i) => (
                ...
                >
                  <Link
                    href="/problems"
                    className="group -mx-4 flex items-start justify-between gap-4 rounded-xl px-4 py-5 transition-colors hover:bg-slate-50 dark:hover:bg-slate-900"
                  >
                    <div className="min-w-0">
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-slate-400 dark:text-slate-500">
                        {t.tags.map((tag) => (
                          <span
                            key={tag}
                            className="rounded-md bg-slate-100 px-2 py-0.5 text-slate-500 dark:bg-slate-800 dark:text-slate-300"
                          >
                            {tag}
                          </span>
                        ))}
