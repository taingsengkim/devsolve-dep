...
export const DiscussionCard: React.FC<DiscussionCardProps> = ({ post, index = 0 }) => {
  ...
  return (
    ...
    >
      ...
      >
        <article className="flex gap-4 rounded-2xl border border-slate-200/80 bg-white p-5 shadow-xs transition-all duration-200 hover:border-slate-300 hover:shadow-md">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-start justify-between gap-3">
              <h3 className="text-lg font-bold text-slate-900 leading-snug line-clamp-2 group-hover:text-blue-600 transition-colors">
                {post.title}
              </h3>
              <span className="shrink-0 rounded-lg bg-slate-100 border border-slate-200/60 px-3 py-1 text-xs font-semibold text-slate-700 whitespace-nowrap">
                {post.topic}
              </span>
            </div>
            ...
            {isShowcase && post.thumbnailUrl ? (
              <div className="mt-3 flex gap-4 items-start">
                <img
                  ...
                  alt={post.title}
                  className="h-28 w-40 object-cover rounded-xl border border-slate-200 shrink-0"
                />
              ...
              <p className="mt-2 text-base text-slate-700 leading-relaxed line-clamp-2">
                {post.description}
              </p>
            )}
            ...
            <div className="mt-3.5 flex flex-wrap gap-1.5">
              {isShowcase && post.techStack
                ? post.techStack.map((tech, i) => (
                    <span
                      key={i}
                      className="rounded-lg bg-purple-50 border border-purple-200/60 px-2.5 py-1 text-xs font-mono font-medium text-purple-800"
                    >
                      {tech}
                    </span>
                  ))
                : post.tags.map((tag, i) => (
                    <span
                      key={i}
                      className="rounded-lg bg-slate-100 border border-slate-200/60 px-2.5 py-1 text-xs font-mono font-medium text-slate-700"
                    >
                      {tag}
                    </span>
                  ))}
              ...
              <div className="flex items-center gap-3 text-sm text-slate-600">
                <div className="flex items-center gap-2">
                  <img
                    ...
                    className="size-6 rounded-full bg-slate-100 border border-slate-200"
                  />
