    ...
    <div className="flex-1 space-y-4">
      ...
      {ROWS.map((row) => (
        <div key={row.key} className="flex items-center justify-between gap-4">
          <span className="text-sm text-[#4d4d4d]">{row.label}</span>
          <Toggle
...
export default function NotificationPreferencesSection({ preferences, onChange }: NotificationPreferencesSectionProps) {
  return (
    <div>
      <p className="text-sm font-semibold text-[#171717]">Notification preferences</p>
      <p className="mt-1 text-sm text-[#4d4d4d]">Choose which notifications you receive.</p>
