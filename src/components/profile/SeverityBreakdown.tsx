...
export default function SeverityBreakdown({ severity }: SeverityBreakdownProps) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-2xs">
      <p className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Severity Breakdown</p>
        ...
        {SEVERITY_ROWS.map((row) => {
          ...
          return (
            <div key={row.key} className="flex items-center gap-3">
              <span className="w-16 shrink-0 text-sm font-semibold text-slate-800 dark:text-slate-200">{row.label}</span>
              ...
              <span className="w-6 shrink-0 text-right text-sm font-bold text-slate-900 dark:text-slate-100">{value}</span>
            </div>
          );
        })}
      </div>
      ...
      <div className="mt-5 grid grid-cols-3 gap-2 border-t border-slate-100 dark:border-slate-800 pt-4 text-center">
        <div>
          ...
          <p className="text-xs font-semibold text-slate-600 dark:text-slate-400">Rejected</p>
        </div>
        <div>
          <p className="text-lg font-bold text-amber-500">{severity.duplicate}</p>
          <p className="text-xs font-semibold text-slate-600 dark:text-slate-400">Duplicate</p>
        </div>
        <div>
          <p className="text-lg font-bold text-emerald-600 dark:text-emerald-400">{severity.retests}</p>
          <p className="text-xs font-semibold text-slate-600 dark:text-slate-400">Retests</p>
        </div>
      </div>
    </div>
  );
