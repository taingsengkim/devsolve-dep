...
export default function SeverityBreakdown({ severity }: SeverityBreakdownProps) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Severity Breakdown</p>
        ...
        {SEVERITY_ROWS.map((row) => {
          ...
          return (
            <div key={row.key} className="flex items-center gap-3">
              <span className="w-16 shrink-0 text-sm font-medium text-slate-600">{row.label}</span>
              ...
              <span className="w-6 shrink-0 text-right text-sm font-semibold text-slate-700">{value}</span>
            </div>
          );
        })}
      </div>
      ...
      <div className="mt-5 grid grid-cols-3 gap-2 border-t border-slate-100 pt-4 text-center">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400">Retests</p>
        </div>
      </div>
    </div>
  );
