...
const TAG_STYLES: Record<CommunityPost["tag"], string> = {
  ...
  Discussion: "bg-blue-50 text-blue-700 border border-blue-100",
};
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex shrink-0 items-center gap-2">
              <span className={`rounded-lg px-2.5 py-0.5 text-xs font-semibold ${TAG_STYLES[post.tag]}`}>{post.tag}</span>
              <button className="text-slate-400 hover:text-slate-600 cursor-pointer" aria-label="More options">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-4 text-xs font-medium text-slate-400">
            ...
            {post.isSolved && (
              <span className="rounded-lg bg-emerald-50 border border-emerald-100 px-2 py-0.5 font-semibold text-emerald-700">Solved</span>
            )}
