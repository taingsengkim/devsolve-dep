...
const TAG_STYLES: Record<CommunityPost["tag"], string> = {
  ...
  Solutions: "bg-emerald-50 text-emerald-700 border border-emerald-100",
...
const STATUS_STYLES: Record<"positive" | "pending", string> = {
  positive: "bg-emerald-50 border border-emerald-100 text-emerald-700",
  pending: "bg-amber-50 border border-amber-100 text-amber-700",
};
...
export default function CommunityPostCard({ post }: CommunityPostCardProps) {
  const body = (
    <div className="flex items-start gap-4">
      ...
      {post.thumbnailUrl && (
        <div className="relative hidden h-20 w-32 shrink-0 overflow-hidden rounded-xl border border-slate-200 bg-slate-100 sm:block dark:border-slate-700 dark:bg-slate-800">
          <Image
  ...
  const surface =
    "block rounded-2xl border border-slate-200/80 bg-white p-5 shadow-2xs transition-shadow hover:shadow-xs dark:border-slate-800 dark:bg-slate-900";
