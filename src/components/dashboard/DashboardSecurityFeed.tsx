    ...
    <div className="bg-white dark:bg-neutral-900 rounded-xl border border-slate-200 dark:border-neutral-800 shadow-xs p-5 flex flex-col justify-between h-full">
      <div>
        <div className="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-neutral-800">
          ...
          <Badge variant="outline" className="text-xs font-normal text-slate-500 shadow-none border-slate-200 dark:border-neutral-800 dark:text-neutral-400">
            Live
          </Badge>
        </div>
          ...
          {feed.map((item, idx) => {
            ...
            return (
              <motion.div
                ...
                className="py-3.5 px-2 hover:bg-slate-50 dark:hover:bg-neutral-800/40 rounded-lg transition-colors flex items-start gap-3"
              >
                <div className={`p-2 rounded-lg shrink-0 mt-0.5 ${config.bg}`}>
                  <Icon className="w-4 h-4" />
                </div>
                <div className="space-y-1 min-w-0 flex-1">
                  <p className="text-sm font-medium text-slate-900 dark:text-neutral-200 leading-snug">
                    {item.title}
                  </p>
